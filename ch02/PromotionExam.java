...
public class PromotionExam {
	...
	public static void main(String[] args) {
		// 자동타입변환(Promotion)은 작은 크기의 타입을 큰 크기 타입으로 자동 변환
		// byte(1) -> short(2) -> int(4)-> long(8)-> float(4) -> double(8)
		//예를 들어 국, 영, 수, 사, 과 과목은 byte -> 총점 int -> 총점 double
		...
		byte sci = 0;
		  ...
		  //키보드로 점수를 입력받는 곳 Scanner input = new Scanner(System.in); 
		  Scanner input = new Scanner(System.in);  //키보드로 입력받는 객체 생성 암기할 것
		  System.out.print("국어 점수 : " ); 
		  kor = input.nextByte(); //스캐너 객체에서 키보드로 입력받은 값을 byte 로 처리
		...
		//byte total = 0 ; 연산시 기본 타입은 int (자동타입변환) 
		int total = 0 ;
