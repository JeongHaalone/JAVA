...
public class ArraySortASCExam {
	...
	public static void main(String[] args) {
		// 버블 정렬 오름차순으로 구현 {IQ 150이상}
		//오름차순은 작은 값부터 큰 값으로 재배열함
		//필수 항목 : temp 임시방
		
		int [] scores = new int[] {79, 88, 91, 33,100, 55, 95}; //scores 변수에 int 배열 7칸 생성하며 정수가 들어있음
		
		System.out.println("=======배열값, 배열초기값 출력=======");
		...
		for (int j = 0 ; j < scores.length-1 ; j++) {
			for (int k =0 ; k < scores.length-1-j ; k++) { 
				...
				scores [k+1] = temp ; //temp의 큰 값이 k +1 번지에 저장
