...
public class BoxExam {
	...
	public static void main(String[] args) {
		...
		// 자동 타입 변환 후에 사용하려면 강제타입으로 변환하여 활용
		// 강제타입변환 테스트
		// 예외 : System.out.println(int String double) 는 자동으로 강제타입변환 코드가 들어있음
		Box box = new Box(); //기본생성자 객체 생성
		box.setObject("문자열"); //자동 타입 변환으로 String 처리 됨
		System.out.println((String)box.getObject());
		String name = (String) box.getObject(); //강제타입변환
		
		Box boxI = new Box();
		boxI.setObject(1234); //Object 타입의 정수 대입
		Integer val = (Integer) boxI.getObject(); //강제타입변환
