...
public class ArithmeticExam {
	...
	public static void main(String[] args) {
		...
		long zl = (long) x*y;
		System.out.println("long 강제자동타입변환한 값을 zl로 출력 :" + zl);
		
		System.out.println("소수점 정확한 계산은 float, double?");
		// 정확한 계산은 부동소수점(실수) 타입은 사용하지 않는 것이 좋다.
		// 사과 1개를 0.1 단위인 10조각으로 보고(piece) 그중 7개의 조각(0.7)을 뺀 3조각(0.3)을 계산
		...
		// 결론 : 돈 계산할때는 double과 같은 실수형으로 사용하지 말것.
		
		System.out.println("-----------------------");
		
		int appleInt = 1; // 사과 1개를 
		int totalPieces = appleInt * 10 ;// 10등분으로 나누지 말고 곱
		int number = 7 ; // 사과 7조각
		int temp = totalPieces - number ; // 사과 10조각 - 7조각
		
		double resultInt = temp /10.0 ; // 3조각 / 10.0 =0.3
